    ...
    public class RaspberryService : IRaspberryService
    {
        ...
        public Task TakeLibcameraPhotoAsync(string filename, Size? size, CancellationToken ct)
        {
            ...
            try
            {
                ...
                string resolution = size.HasValue ? $"--width {size.Value.Width} --height {size.Value.Height}" : string.Empty;
                string arguments = $" {settings.RaspiStillAdditionalArguments} -o {filename} {resolution}";
                logger.LogDebug().WithCategory(LogCategory.System).WithMessage($"Invoking 'libcamera-still {arguments}'").Commit();
                Process proc = Process.Start("libcamera-still", arguments);
                if (proc.HasExited)
                {
                    proc.Dispose();
                    tcs.SetResult(null);
